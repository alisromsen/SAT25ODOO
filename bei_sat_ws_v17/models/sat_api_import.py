...
def get_element(element_root, xpath, namespace):
    #_logger.info("BUSCANDO ELEMENTO XML---------------------------")
    #_logger.info(xpath)
    #_logger.info(namespace)
    element = element_root.find(xpath, namespace)

    #_logger.info(element)
    #_logger.info("99999999999999999999999999999999999999999999999999")
    if element is None:
...
class SAT:
    ...
    def convert_key_cer_to_pem(self, key, password, *args):
        ...
        with tempfile.NamedTemporaryFile('wb', suffix='.key', prefix='edi.mx.tmp.') as key_file, \
            ...
            pwd_file.flush()
            command = ('openssl pkcs8 -in %s -inform der -outform pem -out %s -passin file:%s' % (
                key_file.name, keypem_file.name, pwd_file.name)).split()
            try:
                subprocess.run(command, check=True, capture_output=True)
            except subprocess.CalledProcessError as error:
                message = (error.stderr or b'').decode('utf-8', errors='ignore')
                raise ValidationError(f"Unable to convert SAT key to PEM format. {message}")
            key_pem = keypem_file.read()
        return key_pem

    def check_response(self, response: requests.Response, result_xpath, external_nsmap):
        _logger.info("..C H E C K    R E S P O N S E .............................................")
        ...
        _logger.info(response_xml)
        if response.status_code != requests.codes['ok']:
            fault_details = response_xml.xpath('//*[local-name()="faultstring"]/text()')
            detail_message = fault_details[0] if fault_details else response.text[:300]
            raise Exception(f"SAT response error (HTTP {response.status_code}): {detail_message}")

        return get_element(response_xml, result_xpath, external_nsmap)

    def get_headers(self, soap_action, token=False):
        _logger.info(" GET HEADER ---------------------------------------")
        _logger.info(token)
        headers = {
    ...
    def sign(self, esignature_cer_bin, solicitud):
        ...
        body = '<Signature xmlns="http://www.w3.org/2000/09/xmldsig#"><SignedInfo><CanonicalizationMethod ' \
               'Algorithm="http://www.w3.org/TR/2001/REC-xmlc14n-20010315"/><SignatureMethod ' \
               'Algorithm="http://www.w3.org/2000/09/xmldsig#rsasha1"/><Reference URI=""><Transforms><Transform ' \
    ...
    def prepare_soap_download_data(self, esignature_cer_bin, arguments, body, xpath):
        _logger.info("PREPARANDO solicitud* ......prepare_soap_download_data")
        _logger.info(arguments)
        _logger.info(body)
        _logger.info(xpath)
        _logger.info("88888888888888888888888888888888888888888888888888")
        ...
        _logger.info(element_root)
        solicitud = get_element(element_root, xpath, internal_nsmap)

        try:
            for key in arguments:
                # if key == 'RfcReceptores':
                #     _logger.info("---------SI RECEPTORES 0000000000000000000000000000")
            ...
            _logger.info("111118---81111111111111111**111111111111111111111111111111")
    ...
    def prepare_soap_download_data2(self, esignature_cer_bin, arguments, body, xpath):
        _logger.info("PREPARANDO solicitud* ......prepare_soap_download_data222222")
        #_logger.info(arguments)
        #_logger.info(body)
    ...
    def soap_generate_token(self, certificate: crypto.X509, private_key: crypto.PKey):
        _logger.info("soap_generate_token ***************")
        _logger.info("*******************************************")
        _logger.info("****************T O K E N ***************************")
        _logger.info("*******************************************")
        ...
        date_expires = date_created + timedelta(seconds=600)
    ...
    def soap_request_download(self, token, date_from=None, date_to=None, rfc_emisor=None, tipo_solicitud='CFDI',
                              tipo_comprobante=None, rfc_receptor=None, estado_comprobante='Vigente',
                              rfc_a_cuenta_terceros=None, complemento=None, uuid=None):

        _logger.info(
            "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% SOLICITUD SOLICITA DESCARGA----- soap_request_download %%%%%%%%%%%%%%%%%%%%%%%%%%%%%5555")
        ...
        arguments = {
            ...
            # 'TipoFormato': "xml"
            # 'RfcReceptores': self.holder_vat,
            # 'RfcACuentaTerceros': rfc_a_cuenta_terceros,
            # 'Complemento': complemento,
            # 'UUID': uuid,
        }
        _logger.info("5555555555555555555555555actualizado5555555555555555555555555555555555555")
        _logger.info(arguments)
        if rfc_emisor:
        ...
        body = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" ' \
               ...
               '<soapenv:Header/>' \
               '<soapenv:Body><des:SolicitaDescargaRecibidos>' \
               '<des:solicitud></des:solicitud>' \
               '</des:SolicitaDescargaRecibidos></soapenv:Body></soapenv:Envelope>'
        #_logger.info(body)

        # body = '<s:Envelope xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx" ' \
        #        'xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"><s:Header/><s:Body><des:SolicitaDescargaRecibidos>' \
        #        '<des:solicitud><des:RfcReceptores><des:RfcReceptor/></des:RfcReceptores></des:solicitud>' \
        #        '</des:SolicitaDescargaRecibidos></s:Body></s:Envelope>'

        xpath = 'soapenv:Body/des:SolicitaDescargaRecibidos/des:solicitud'
        ...
        _logger.info(".................PREPARA DESCARGA..................................")
        _logger.info(soap_request)
        cabeza = self.get_headers(soap_action, token)
        _logger.info(".................T O K E N....................................")
        #_logger.info(token)
        #_logger.info(cabeza)
        ...
        if cabeza:
            ...
            _logger.info("FORMULANDO communication-----")
            _logger.info(communication)
            if communication:
                element_response = self.check_response(communication, result_xpath, external_nsmap)
                _logger.info("88888888888888 R E S P O N S E 8888888888888888888888888888888888888")
                _logger.info(element_response)
                #_logger.info("888888888888888888888888888888888888888888888888888")
                ret_val = {
                ...
                _logger.info("-----NO COMUNNICATION------")
        else:
            _logger.info("-----NO Cabeza------")
        return ret_val

    def soap_verify_package(self, signature_holder_vat, id_solicitud, token):
        _logger.info("******   VERIFICADO PAQUETE *******soap_verify_package*********")
        _logger.info(signature_holder_vat)
        _logger.info(id_solicitud)
        _logger.info(token)
        ...
        cer = base64.b64encode(crypto.dump_certificate(crypto.FILETYPE_ASN1, self.certificate))
        soap_request = self.prepare_soap_download_data(cer, arguments, body, xpath)
        _logger.info("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
        _logger.info(token)
        if not token:
            _logger.info("NO HAY TOKENNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN")
        ...
        element_response = self.check_response(communication, result_xpath, external_nsmap)
        _logger.info("1000010101010100000000 R E S P VERIFICA PAQUETE 0000000000000001010101010101010101001010101")
        _logger.info(communication)
        ret_val = {
            'cod_estatus': element_response.get('CodEstatus'),
    ...
    def soap_download_package(self, signature_holder_vat, id_paquete, token):
        _logger.info(" 00000000000000 SOAP DESCARGA PAQUTE 000000000000000000000000")
        _logger.info(" ++ petición hacia el servicio con los parámetros ++")
        ...
        cer = base64.b64encode(crypto.dump_certificate(crypto.FILETYPE_ASN1, self.certificate))
        soap_request = self.prepare_soap_download_data2(cer, arguments, body, xpath)
        communication = requests.post(
            soap_url,
            soap_request,
            headers=self.get_headers(soap_action, token),
