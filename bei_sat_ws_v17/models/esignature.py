...
def convert_key_cer_to_pem(key, password):
    ...
    with tempfile.NamedTemporaryFile('wb', suffix='.key', prefix='edi.mx.tmp.') as key_file, \
        ...
        pwd_file.flush()
        command = (KEY_TO_PEM_CMD % (key_file.name, keypem_file.name, pwd_file.name)).split()
        try:
            subprocess.run(command, check=True, capture_output=True)
        except subprocess.CalledProcessError as error:
            message = (error.stderr or b'').decode('utf-8', errors='ignore')
            raise ValidationError(_('The certificate key and/or password is/are invalid. %s') % message)
